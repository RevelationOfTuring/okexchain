...
func queryProposal(ctx sdk.Context, path []string, req abci.RequestQuery, keeper Keeper) ([]byte, sdk.Error) {
	...
	if err != nil {
		return nil, types.ErrUnknownRequest(keeper.Codespace(),"incorrectly formatted request data")
	}
...
func queryDeposit(ctx sdk.Context, path []string, req abci.RequestQuery, keeper Keeper) ([]byte, sdk.Error) {
	...
	err := keeper.Cdc().UnmarshalJSON(req.Data, &params)
	if err != nil {
		return nil, types.ErrUnknownRequest(keeper.Codespace(), sdk.AppendMsgToErr("incorrectly formatted request data", err.Error()))
	}
...
func queryProposals(ctx sdk.Context, path []string, req abci.RequestQuery, keeper Keeper) ([]byte, sdk.Error) {
	...
	if err != nil {
		common.ErrMarshalJSONFailed(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
	}
	return bz, nil
}
